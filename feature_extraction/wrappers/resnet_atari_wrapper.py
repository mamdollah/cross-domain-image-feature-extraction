...
class WarpFrame(gym.ObservationWrapper[np.ndarray, int, np.ndarray]):
    ...
    def __init__(self, env: gym.Env, width: int = 224, height: int = 224) -> None:
        super().__init__(env)
        self.width = width
        self.height = height
...
class ResnetAtariWrapper(gym.Wrapper[np.ndarray, int, np.ndarray, int]):
    """
    Atari 2600 preprocessings adapted for ResNet models.
    ...
    * Resize to a square image: 224x224 by default
    * Default RGB observation
